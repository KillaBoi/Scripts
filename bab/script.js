...
log('-> We do assume 20 games is the maximum streak without 1.4x so...');
...
engine.on('GAME_STARTING', function () {
    log('');
    log('NEW GAME')
    log('Games since no 1.4x: ' + currentRedStreak + '. You can handle: ' + gamesTheBotCanHandle + ' games without 1.4x');
    log('Actual profit using the script: ' + userProfitInSatoshis / 100 + ' bits. Got ' + numberOf2xCashedOut + ' times 1.4x');
    ...
    if (redStreakOverLimit || minutesLeft > 0 || gamesLeft > 0 || !wonLastGame) {
        ...
        if(minutesLeft > 0 || gamesLeft > 0 || !wonLastGame){
            ...
            engine.bet(currentBetInSatoshis, 1.4);
        ...
        if (calculatedGamesToWait < 1) {
            ...
            log('Waiting for ' + calculatedGamesToWait + ' more games with no 1.4x');
        }
    }
})
